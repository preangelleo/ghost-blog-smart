...
def compare_both_approaches():
    ...
    try:
        ...
        print(
            f"Class result: {class_result['success']} - {class_result.get('message', '')}"
        )
    ...
    try:
        ...
        print(
            f"Function result: {function_result['success']} - {function_result.get('message', '')}"
        )
    ...
    print(
        f"Function-based: {'✅ Success' if function_result['success'] else '❌ Failed'}"
    )
...
def quick_start_examples():
    ...
    print("\n💡 RECOMMENDATION:")
    print(
        "   - Use CLASS-BASED approach if you're building applications or using with code assistants"
    )
    print(
        "   - Use FUNCTION-BASED approach for scripts, notebooks, or when you need granular control"
    )
...
def example_7_smart_gateway_with_flux():
    ...
    result = smart_blog_gateway(
        raw_content, status="draft", preferred_language="English", **DEFAULT_CONFIG
    )
...
def example_10_get_posts():
    ...
    result = get_ghost_posts(limit=5, status="all", **DEFAULT_CONFIG)
...
def example_11_advanced_post_search():
    ...
    result = get_ghost_posts_advanced(
        search="AI", status="all", limit=5, **DEFAULT_CONFIG
    )
