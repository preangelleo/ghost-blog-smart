...
def standardize_response(success=True, data=None, error=None, message=None, status_code=200):
...
def _extract_ghost_credentials():
    ...
    ghost_api_key = request.headers.get('X-Ghost-API-Key') or os.environ.get('GHOST_ADMIN_API_KEY')
    ghost_api_url = request.headers.get('X-Ghost-API-URL') or os.environ.get('GHOST_API_URL')
    gemini_api_key = request.headers.get('X-Gemini-API-Key') or os.environ.get('GEMINI_API_KEY')
    replicate_api_key = request.headers.get('X-Replicate-API-Key') or os.environ.get('REPLICATE_API_TOKEN')
