...
class TestGhostBlogSmartAPI:
    ...
    def test_get_posts_advanced_search(self, client, auth_headers):
        ...
        response = client.get('/api/posts/advanced?limit=5&status=published', headers=auth_headers)
        assert response.status_code == 200
    ...
    def test_create_post_valid(self, client, auth_headers):
        ...
        assert response.status_code in [200, 400]  # 400 is OK if test mode blocks creation
    ...
    def test_smart_create_without_auth(self, client):
        ...
        smart_data = {
            'user_input': 'Write about AI benefits'
        }
        
        response = client.post('/api/smart-create',
    ...
    def test_batch_post_details(self, client, auth_headers):
        ...
        if list_data['data']['posts']:
            ...
            response = client.post('/api/posts/batch-details',
    ...
    def test_update_post_without_auth(self, client):
        ...
        response = client.put('/api/posts/some_id',
    ...
    def test_date_pattern_search(self, client, auth_headers):
        ...
        response = client.get('/api/posts/search/by-date-pattern?pattern=2024&limit=5', 
                            headers=auth_headers)
        assert response.status_code == 200
